...
def parse_args():
    ...
    p.add_argument("--out", type=str, default="test2_original.mp4")
    return p.parse_args()
...
RED_X_RETURN_START = 30.0    # == X at T_TURN_START (continuity)
...
def clip_near(points_cam, near=NEAR_PLANE):
    """Sutherland-Hodgman clip of a convex polygon (camera-space points)
    against the z_cam > near plane, so faces straddling the camera are cut
    cleanly instead of being dropped whole or blown up by a near-zero
    perspective divide."""
    if not points_cam:
        return []
    out = []
...
def project_poly(points, cam_pos, cam_target, cam_up, fov_deg, width, height):
    projected = []
    for pt in points:
        r = project(pt, cam_pos, cam_target, cam_up, fov_deg, width, height)
        if r is None:
            return None
        projected.append(r)
    return projected
...
def render_frame(t, width, height):
    ...
    for name, pts, color in all_faces:
        proj = project_poly(pts, cam_pos, cam_target, cam_up, FOV_DEG, width, height)
        if proj is None:
            continue
